...
@bot.on(events.NewMessage(pattern="/tmmisslyra"))
async def tmp(event):
    if event.reply_to_msg_id:
        start = time.time()
        url = await event.get_reply_message()
        ...
        try:
            orta = await event.respond("Uploading to MissLyraRobot...")
            download_link = await send_to_tmp_async(file_path)

            str(time.time() - start)
            await orta.edit(
                f"File Successfully Uploaded to MissLyraRobot.\n\nLink 👉 {download_link}\n\nUploaded by *MissLyraRobot*"
            )
        except Exception as e:
            traceback.print_exc()
            print(e)
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            await bot.send_file(
                ...
                dosya,
                force_document=True,
                caption=f"Uploaded By *MissLyraRobot*",
            )
        except Exception as e:
            traceback.print_exc()
